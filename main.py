import streamlit as st
 ...
 def load_summarizer_model():
  with st.spinner("Cargando modelo de resumen multilingüe..."):
  try:
  # Modelo mT5 para resumen multilingüe (puedes probar con variantes más grandes si tienes recursos)
  summarizer = pipeline("summarization", model="google/mt5-base-summarization")  # O "google/mt5-large-summarization"
  return summarizer
  except Exception as e:
 ...
 if summarizer is not None: # Solo muestra el botón si el modelo se cargó correctamente
  ...
  all_summaries_from_chunks.append(chunk_summary)
  ...
  final_summary_output = summarizer(combined_summaries_text, max_length=500, min_length=150, do_sample=False)[0]['summary_text']
  final_summary = final_summary_output
  else:
  ...
  summary_output = summarizer(text_content, max_length=500, min_length=100, do_sample=False)[0]['summary_text']
